/*
Интерфейс управления сканированием
*/
...
extension ViewController {
    ...
    var state: State {
        ...
        set {
            // Проверка условий для изменения состояния
            var newState = newValue
            switch newValue {
            case .startARSession:
                break
            case .notReady:
                // Смена готовности, если трекинг поймался
            ...
            case .scanning:
                // Смена готовности, если трекинг невозможен
                if let camera = self.sceneView.session.currentFrame?.camera {
                    switch camera.trackingState {
                    case .normal:
                        break
    ...
    func switchToNextState() {
        ...
        case .testing:
            // Проверка сканирования, отображение Share
